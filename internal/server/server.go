...
func (s *Server) handleConnect(w http.ResponseWriter, r *http.Request) {
	...
	bars, err := serialpkg.OpenLeo485(rec.P.SERIAL, rec.P.BARS)
	if err != nil {
		s.writeJSON(w, 400, APIError{Error: err.Error()})
		return
	}
...
func (s *Server) handleCalPlan(w http.ResponseWriter, r *http.Request) {
	...
	steps, _, err := modern.BuildCalibrationPlan(p, bars.NLCs)
	if err != nil {
		s.writeJSON(w, 500, APIError{Error: err.Error()})
		return
	}
...
func (s *Server) handleCalStartStep(w http.ResponseWriter, r *http.Request) {
	...
	steps, nloads, err := modern.BuildCalibrationPlan(p, bars.NLCs)
	if err != nil {
		s.writeJSON(w, 500, APIError{Error: err.Error()})
		return
	}
	...
	go func() {
		flat, err := modern.SampleADCs(ctx, bars, p.IGNORE, p.AVG, func(u modern.SampleUpdate) {
		...
		if step.Kind == modern.CalStepZero {
			s.dev.calAd0 = modern.UpdateMatrixZero(flat, calibs, nlcs)
			s.dev.calAdv = matrix.NewMatrix(nloads, nbars*nlcs)
		} else if s.dev.calAdv != nil {
			s.dev.calAdv = modern.UpdateMatrixWeight(s.dev.calAdv, flat, step.Index, nlcs)
		}
		s.dev.calReceived++
		...
		if err := modern.ComputeZerosAndFactors(s.dev.calAdv, s.dev.calAd0, p); err != nil {
			s.wsCal.Broadcast(WSMessage{Type: "error", Data: map[string]string{"error": err.Error()}})
			return
		}
		...
		err = modern.FlashParameters(ctx, bars, p, func(pr modern.FlashProgress) {
			s.wsCal.Broadcast(WSMessage{
				Type: "flashProgress",
				Data: map[string]interface{}{
